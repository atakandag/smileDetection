...
def main():
    images = load_images("./GENKI-R2009a/Subsets/GENKI-4K/files")  #load all images into an array
    labels = load_labels("./GENKI-R2009a/Subsets/GENKI-4K/GENKI-4K_Labels.txt")  #load all labels into an array
    ...
    model = keras.Sequential([
        keras.layers.Conv2D(filters=32, kernel_size=convSize, activation="relu", input_shape=train_x.shape[1:],padding="same"),
        keras.layers.BatchNormalization(),
        keras.layers.Conv2D(filters=32, kernel_size=convSize, activation="relu", input_shape=train_x.shape[1:],
                            padding="same"),
        keras.layers.BatchNormalization(),
        keras.layers.MaxPooling2D(pool_size=(2, 2), padding="same"),
        keras.layers.Conv2D(filters=64, kernel_size=convSize, activation="relu", input_shape=train_x.shape[1:],
                            padding="same"),
        keras.layers.BatchNormalization(),
        keras.layers.Conv2D(filters=64, kernel_size=convSize, activation="relu", input_shape=train_x.shape[1:],
                            padding="same"),
        keras.layers.BatchNormalization(),
        keras.layers.MaxPooling2D(pool_size=(2, 2), padding="same"),
        keras.layers.Conv2D(filters=128, kernel_size=convSize, activation="relu", input_shape=train_x.shape[1:],
                            padding="same"),
        keras.layers.BatchNormalization(),
        keras.layers.Conv2D(filters=128, kernel_size=convSize, activation="relu", input_shape=train_x.shape[1:],
                            padding="same"),
        keras.layers.BatchNormalization(),
        keras.layers.MaxPooling2D(pool_size=(2, 2), padding="same"),
        keras.layers.Conv2D(filters=256, kernel_size=convSize, activation="relu", input_shape=train_x.shape[1:],
                            padding="same"),
        keras.layers.BatchNormalization(),
        keras.layers.Conv2D(filters=256, kernel_size=convSize, activation="relu", input_shape=train_x.shape[1:],
                            padding="same"),
    ...
    history = model.fit(train_x, train_y, epochs=50, batch_size=32, validation_data=(val_x, val_y)) #train the model for 50 epochs
